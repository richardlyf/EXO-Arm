...
static void printToFile(LPCWSTR data, const char* nameFile) {
	errno_t err;
	err = fopen_s(&stream, nameFile, "a+");
	char buffer[1024];

	if (stream != NULL)
	{		
		wcstombs(buffer, data, 1024);
		fputs(buffer, stream);
		fputs("\n", stream);
		fclose(stream);
	}
...
static void AlgoSdkCallback(sNSK_ALGO_CB_PARAM param) {
	...
	{
		switch (param.param.index.type) {	//prints out the types of data?
			case NSK_ALGO_TYPE_ATT:
			{
				...
				OutputLog(buffer);
				printToFile(buffer, "EEG_ATT_data.txt"); //write data into att data file
				break;
			}
			case NSK_ALGO_TYPE_MED:
			{
				...
				OutputToEditBox(buffer);
				OutputLog(buffer);
				printToFile(buffer, "EEG_MED_data.txt"); //write data into med data file
				break;
			}
			case NSK_ALGO_TYPE_BLINK:
			{
				...
				OutputToEditBox(buffer);
				OutputLog(buffer);
				printToFile(buffer, "EEG_BLINK_data.txt");
				break;
			}
			case NSK_ALGO_TYPE_BP:
			{
				...
				OutputToEditBox(buffer);
				OutputLog(buffer);
				printToFile(buffer, "EEG_BP_data.txt"); //write in all bandpass data
				break;
			}
		}
	}
